...
export function bufferToBlob(buffer: ArrayBuffer, mimeType: string = 'audio/wav'): Blob {
...
export async function generateSpeech(userId: string, text: string): Promise<ReadableStream<Uint8Array> | Blob> {
  try {
    ...
    if (typeof ReadableStream !== 'undefined' && 'body' in Response.prototype && window.MediaSource) {
      ...
      try {
        const response = await fetch(`${apiUrl}?text=${encodeURIComponent(text)}&user_id=${encodeURIComponent(userId)}`, {
          method: 'GET',
        });
